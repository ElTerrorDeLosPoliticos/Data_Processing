...
def obtain_horas(fecha):
    try:
        ...
        try:
            ...
            dt_1 = datetime.strptime(fecha, '%H:%M:%S')
    return dt_1

def obtain_date(fecha):
    if fecha != None and isinstance(fecha, str) and (fecha != ' '):
        dt_1 = obtain_horas(fecha)
    else:
        return 'vacio'
    return dt_1
...
def restar_horas(x):
    ...
    if (x[0] != None) and (isinstance(x[0], str)) and (x[0] != ' '):
        dt_1 = obtain_horas(x[0].strip())
    else:
        dt_1 = 0
    # Revisa si no es null y si es str
    if (x[1] != None) and (isinstance(x[1], str)) and (x[1] != ' '):
        dt_2 = obtain_horas(x[1].strip())
    else:
        dt_2 = 0
...
def transform_visitantes(df_c):
    df = df_c.copy()
    df[['Cargo', 'Tipo_Documento']]=df[['Cargo','Tipo_Documento']].fillna(value='vacio')
    ...
    df.loc[df['Visitante'] =='carlos eduardo felix quispe',('Visitante')] = 'felix quispe carlos eduardo'
    df.loc[df['Visitante'] =='freddy jose torres vela',('Visitante')] = 'torres vela freddy jose'
    df.loc[df['Visitante'] =='gabriel ernesto ramos la torre',('Visitante')] = 'ramos la torre gabriel ernesto'
    df.loc[df['Visitante'] =='ocampo zu ntilde iga antonio',('Visitante')] = 'ocampo zuñiga antonio'
    df.loc[df['Visitante'] =='jose lidio orihuela quiliano',('Visitante')] = 'orihuela quiliano jose lidio'
    df.loc[df['Visitante'] =='henry alexis chavez ancajima',('Visitante')] = 'chavez ancajima henry alexis'
    df.loc[df['Visitante'] =='francisco martin gavidia arrascue',('Visitante')] = 'gavidia arrascue francisco martin'
    df.loc[df['Visitante'] =='ernesto puga leon',('Visitante')] = 'puga leon ernesto'
    ...
    entidad = pd.pivot_table(df , values = 'entidad', index = ['id'], aggfunc = lambda x: x.unique())
    entidad.columns = ['entidades']
    ...
    to_df = pd.concat([new_df, fechas, recurrencia,tiempo_total, min_dia, max_dia, reunion_sin_tiempo, n_visitados, n_entidad, entidad, tipo_documento, documento, cargos, n_oficinas, oficinas], axis = 1)
...
def transform_visitados(df_c):
    ...
    new_df.index = new_df['id']
    new_df.drop_duplicates(inplace= True)
    to_df = pd.concat([new_df, fechas, recurrencia,tiempo_total, min_dia, max_dia, reunion_sin_tiempo, n_visitados, n_entidad, entidad,cargos, n_oficinas], axis = 1)
    to_df.drop(['id'], axis = 1, inplace = True)
    to_df['tiempo_reunion/n_fechas (h)'] = to_df['Tiempo_reuniones(min)']/to_df['fechas']/60.0
    to_df['tiempo_reunion/n_fechas (h)'] = to_df['tiempo_reunion/n_fechas (h)'].apply(lambda x : round(x, 2))
    to_df.sort_values(by = ['fechas'], ascending = False, inplace = True)
...
def save_fig(fig_id, tight_layout=True, fig_extension="png", resolution=300):
    IMAGES_PATH = os.getcwd() + '/imagenes/visitas_ministerios'
